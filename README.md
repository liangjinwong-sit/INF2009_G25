...
GymPulse is a real-time gym equipment monitoring system built on a 3-Raspberry Pi edge computing architecture. It uses computer vision to detect people, track their movement across equipment zones, and determine whether gym machines are actively in use — all processed at the edge with no cloud dependency.

A live web dashboard displays zone occupancy, session timers, and usage history. Telegram alerts notify gym staff when equipment has been occupied beyond a configurable threshold.
...
All three Raspberry Pis communicate over a local network. RPi 1 publishes vision state to RPi 3's MQTT broker at 1 Hz. RPi 3 serves a live dashboard accessible from any device on the same network.
...
- **Hardware**: Raspberry Pi 4/5, USB webcam, PIR sensor
